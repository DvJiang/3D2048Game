...
static float red = 1.0f,green = 1.0f, blue = 1.0f;
static float sred = 1.0f,sgreen = 1.0f, sblue = 1.0f;
static float x = 0.0f,y = 0.0f,z = 0.0f;
static float win_scale = 2.5;
static float camera_lenth = 8.0f;
static float doki_scale = 1.0f,doki_scale_i = 1,doki_speed = 2,doki_s = 0.3;
static bool self_span = 0,doki = 1,lineStyle = 1,show_set = 0,colorStyle = 1,sizeStyle = 0,num_flag = 1,help_flag = 0;
GLuint mainw,helpw;
Game game;
...
void help() {
    float posx = -4.0, posy = 3.0, lineSpace = -0.5, num = 3.0;
    ...
    num++;strShow("Use WASD to control the game,and use direction key to control the camera view.",0.0,posy + num * lineSpace,posx);
...
void gameOver() {
    ...
    if (c == 0) {
        ...
        strShow("2048 Get",-1.0,posy + (num - 2) * lineSpace,posx + 1.5);
        strShow("You Win!",0.0,posy + num * lineSpace,posx + 2.0);
        strShow(strAnd("score:",int2str(game.score())),0.0,posy + num * lineSpace,posx + 4.0);
        strShow("Click \'Restart\' from the menu to restart game",0.0,posy + 12.0 * lineSpace,posx + 4.0);
    }
}
